...
def fig4_combined_accessibility_inequality(gdf: gpd.GeoDataFrame) -> None:
    # ------------------
    # ECDF data preparation
    # ------------------
    dist_col = find_col(
        gdf,
        ["dist_to_accessible_A", "dist_to_accessible_A_m", "dist_accessible_A_m", "nearest_accessible_A_m"],
    ...
    # ==================
    ax0 = axes[0]
    if not d.empty and total_pop > 0:
        x_ecdf = np.insert(d[dist_col].to_numpy(), 0, 0)
        y_ecdf = np.insert(np.cumsum(d["population"].to_numpy()) / total_pop, 0, 0)
        ax0.plot(x_ecdf, y_ecdf, color="#1F77B4", linewidth=2.8, label="Level A UGBS")
        ...
        if dist_col_ab is not None:
            ...
            if not d_ab.empty:
                x_ab = np.insert(d_ab[dist_col_ab].to_numpy(), 0, 0)
                y_ab = np.insert(np.cumsum(d_ab["population"].to_numpy()) / total_pop, 0, 0)
                ax0.plot(x_ab, y_ab, color="#B2182B", linewidth=2.3, linestyle="--", label="Level A+B UGBS")
        
        # Legend for ECDF
        ax0.legend(loc="lower right", frameon=True, fontsize=9.5, facecolor="white", edgecolor="#CCCCCC")

        # Threshold vertical lines and labels
        thresholds = [300, 500, 1000]
        summary = []
        for thr in thresholds:
            if thr <= cutoff:
                pct = data.loc[reachable & (data[dist_col] <= thr), "population"].sum() / total_pop * 100
                summary.append((thr, pct))
                ax0.axvline(thr, color="#777777", linestyle=":" if thr != 500 else "--", linewidth=1.0)
    ...
    ax0.set_xlabel("Network walking distance to nearest UGBS (m)", fontsize=11.5)
    ax0.set_ylabel("Cumulative share of residential population", fontsize=11.5)
    ax0.grid(True, linestyle="--", alpha=0.25)
    add_panel_label(ax0, "A")
    ax0.set_title("Population-weighted walking distance", fontsize=11.5, fontweight="bold")
    ...
    if len(x_a):
        ...
        ax1.fill_between(x_a, x_a, y_a, color="#1F77B4", alpha=0.10)
    ...
    ax1.set_xlabel("Cumulative share of population", fontsize=11.5)
    ax1.set_ylabel("Cumulative share of accessible UGBS area", fontsize=11.5)
    ax1.grid(True, linestyle="--", alpha=0.25)
    ax1.legend(loc="upper left", fontsize=9.5, frameon=True, facecolor="white", edgecolor="#CCCCCC")
    add_panel_label(ax1, "B")
    ax1.set_title("Lorenz curves of accessible UGBS area", fontsize=11.5, fontweight="bold")

    fig.tight_layout()
    save_fig(fig, "fig4_combined_accessibility_inequality.png")
...
def run() -> None:
    ...
    fig4_combined_accessibility_inequality(gdf)
